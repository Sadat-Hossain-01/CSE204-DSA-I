#include <iostream>
#include <vector>
using namespace std;

const int INF = 2e9;
#define endl "\n"

inline void min_self(int &a, int b) { a = min(a, b); }

template <typename T>
void printTable(const vector<vector<T>> &vec) {
  for (int i = 0; i < vec.size(); i++) {
    for (int j = 0; j < vec[i].size(); j++) cerr << vec[i][j] << " ";
    cerr << endl;
  }
}

int solve(vector<vector<int>> &dp, const vector<vector<int>> &cost, int n,
          int current, int mask) {
  int &ans = dp[current][mask];
  if (ans != -1) return ans;
  if (mask == 0) return ans = 0;
  for (int city = 0; city < n; city++) {
    if (!(mask & (1 << city))) {
      int charge = 0;
      for (int any = 0; any < n; any++)
        if (mask & (1 << any)) charge += cost[city][any];
      min_self(ans, solve(dp, cost, n, city, mask | (1 << city)));
    }
  }
}

int main() {
  int n;
  cin >> n;
  vector<vector<int>> cost(n, vector<int>(n));
  for (int i = 0; i < n; i++)
    for (int j = 0; j < n; j++) cin >> cost[i][j];
  vector<vector<int>> dp(n, vector<int>(1 << n, -1));

  //   for (int i = 0; i < n; i++) {
  //     dp[i][1 << i] = cost[i][i];
  //     for (int mask = 0; mask < (1 << n); mask++) {
  //       for (int bit = 0; bit < n; bit++) {
  //         if (bit == i) continue;
  //         if (!(mask & (1 << bit))) {
  //           // city bit is unvisited
  //           int charge = 0;
  //           for (int j = 0; j < n; j++) {
  //             if (mask & (1 << j)) charge += cost[bit][j];
  //           }
  //           min_self(dp[bit][mask | (1 << bit)], dp[i][mask] + charge);
  //         }
  //       }
  //     }
  //   }

  int ans = INT_MAX;
  for (int i = 0; i < n; i++) min_self(ans, dp[i][(1 << n) - 1]);
  printTable(dp);
  cout << ans << endl;
}